...
plt.figure(figsize=(2.5,1.81))  # Increase the figure size
...
for behavior in new_behavior_order:
    ...
    plt.plot(mean, color=colors[b], label=behavior, ls='--', lw=3, marker='o', ms=2, markerfacecolor='white')
...
for i, label in enumerate(group_labels):
    plt.text(group_positions[i], -0.2, label, ha='center', va='center', fontsize=18, transform=plt.gca().transAxes)
...
for b in behaviors_of_interset:
    ...
    plt.savefig(output_folder+'/SSD_stat_'+behaviors[b]+'.pdf',dpi=300)
    plt.close()
...
colors_prime = ["#d3d3d3","#a9a9a9","#808080","#ffa500","#ff8c00","#ff6347","#e60000","#990000"]

# Set figure size to 1.8x1.8 inches
plt.figure(figsize=(1.8, 1.8))
for i in range(num_of_trials):
    if i < num_of_trials - 1:
        plt.plot(np.arange(i, i + 2), mean[i:i + 2], ls='--', marker='o',markersize=3, lw=1, markerfacecolor=colors_prime[i], markeredgecolor=colors_prime[i], c=colors_prime[i], alpha=0.7)
    else:
        plt.plot(np.arange(i, i + 1), mean[i:i + 1], ls='--', marker='o',markersize=3, lw=1, markerfacecolor=colors_prime[i], markeredgecolor=colors_prime[i], c=colors_prime[i], alpha=0.7)

for i in range(num_of_trials):
    plt.errorbar(i, mean[i], yerr=stderr[i], color=colors_prime[i], capsize=2, capthick=1, ls='', alpha=0.7)
...
group_positions = [0.18, 0.7]
for i, label in enumerate(group_labels):
    plt.text(group_positions[i], -0.2, label, ha='center', va='center', fontsize=SMALL_SIZE, transform=plt.gca().transAxes)

# Adjust y-axis labels to only show 0, 50, 100
plt.yticks([0, 50, 100])

sns.despine()
# plt.xlabel('Day', labelpad=12)
plt.ylabel('Distance Travelled (m)')
# plt.savefig(output_folder + 'distance_travelled.pdf', dpi=300, bbox_inches='tight')
plt.show()
# plt.close()
...
plt.savefig(output_folder+'cocaine_long_term_effect.pdf',dpi=300)
plt.close()
...
nx.draw(G, pos=nx.circular_layout(G), edge_color=edge_colors, node_color=colors, width=list(weights), node_size=saline_time_spent,
        arrowstyle='->'
        , arrowsize=15, connectionstyle='arc3, rad = 0.1', with_labels=False)
plt.savefig(output_folder + 'states_actions_mean_saline_days.pdf', dpi=300)
plt.close()
...
for t_idx in np.arange(num_of_subset_trials-3):
    ...
    plt.savefig(output_folder + 'states_actions_diff_' + trials_subset[t_idx+3] + '.pdf', dpi=300)
    plt.close()
...
for b in range(num_of_behaviors):
    ...
    # plt.legend()
    plt.xlim(0, longest_bout)
    plt.xticks(np.arange(0, longest_bout + 1, 100))
    sns.despine()
    plt.tight_layout()
...
# Define the improved color palette for the behaviors
behavior_colors = {
    'stationary': '#ADD8E6',  # Light blue
    'locomotion': '#5DADE2',  # Medium blue
    'rearing': '#1F77B4',     # Blue
    'body licking': '#1A5276',# Dark blue
    'grooming': '#0000FF',    # Blue
    'wall licking': '#FF6347',# Tomato (orange-red)
    'floor licking': '#B22222',# Firebrick (dark red)
    'back to camera': '#696969', # Dim gray
    'Jump': '#D3D3D3'         # Light gray
}

# Plot the colors
fig, ax = plt.subplots(figsize=(10, 2))
for i, (behavior, color) in enumerate(behavior_colors.items()):
    ax.add_patch(plt.Rectangle((i, 0), 1, 1, color=color))
    ax.text(i + 0.5, 1.1, behavior, ha='center', va='bottom', fontsize=12)

ax.set_xlim(0, len(behavior_colors))
ax.set_ylim(0, 1)
ax.axis('off')

plt.show()
# %%
# Define the improved color palette with a smoother transition from blue to red
behavior_colors = {
    'stationary': '#ADD8E6',  # Light blue
    'locomotion': '#5DADE2',  # Medium blue
    'rearing': '#7B68EE',     # Medium slate blue
    'body licking': '#8A2BE2',# Blue violet
    'grooming': '#FF00FF',    # Magenta
    'wall licking': '#FF6347',# Tomato (orange-red)
    'floor licking': '#B22222',# Firebrick (dark red)
    'back to camera': '#696969', # Dim gray
    'Jump': '#D3D3D3'         # Light gray
...
# Define the improved color palette with a softer transition from blue to red
behavior_colors = {
    'stationary': '#ADD8E6',  # Light blue
    'locomotion': '#87CEFA',  # Light sky blue
    'rearing': '#9370DB',     # Medium purple
    'body licking': '#BA55D3',# Medium orchid
    'grooming': '#DA70D6',    # Orchid
    'wall licking': '#FF7F7F',# Light coral
    'floor licking': '#CD5C5C',# Indian red
    'back to camera': '#696969', # Dim gray
    'Jump': '#D3D3D3'         # Light gray
}
...
for i in range(num_of_trials):
    if i < num_of_trials - 1:
        plt.plot(np.arange(i, i + 2), mean[i:i + 2], ls='--', marker='o', lw=1, markerfacecolor=colors_prime[i], markeredgecolor=colors_prime[i], c=colors_prime[i], alpha=0.7)
    else:
        plt.plot(np.arange(i, i + 1), mean[i:i + 1], ls='--', marker='o', lw=1, markerfacecolor=colors_prime[i], markeredgecolor=colors_prime[i], c=colors_prime[i], alpha=0.7)
