...
for t in stim_days.keys():
    for mouse in stim_days[t]:
        ...
        plt.sca(ax[0])
        plt.ylabel('Stim')
        ax[2].plot(velocity * FPS,c = velocity_color,alpha=.7)
        ...
        plt.suptitle('A2a_opto_splashTest , '+mouse+' , '+t)
...
for t in ['splashTest1p2mW','splashTest2p2mW']:
    ...
    for category in [VDB,nVDB]:
        ...
        if category == VDB:
            plt.title(t+'\nChange in behavior around laser epoch\n Behaviors: Grooming, body licking,wall licking,floor licking')
            plt.savefig(output_folder + '/'+t+'V_graph_VDB'  + '.png', dpi=300)
        else:
            plt.title(t+'\nChange in behavior around laser epoch\n Behaviors: Rearing,Other,BTC,Jump')
            plt.savefig(output_folder + '/'+t+'V_graph_nVDB' + '.png', dpi=300)
        plt.close()
        for behavior in range(num_of_behaviors):
            plt.figure()
            ...
            plt.errorbar(np.arange(3), y=np.mean(stim_epoch_behavior[:, :, :, behavior], axis=(1, 2)),
                         yerr=np.sqrt(
                             np.var(stim_epoch_behavior[:, :, :, behavior]) / (stim_epoch_behavior.shape[1] * stim_epoch_behavior.shape[2])).T,
            ...
            plt.savefig(output_folder + '/'+t+'V_graph_' + behaviors[behavior] + '.png', dpi=300)
            plt.close()
...
for t in ['splashTest1p2mW','splashTest2p2mW']:
    ...
    for category in [VDB,nVDB]:
        ...
        plt.ylim([0,1])
        if category==VDB:
            plt.title(t+'\nDynamics of Change in behavior around laser epoch\n Behaviors: Grooming, body licking,wall licking,floor licking')
            plt.savefig(output_folder + '/'+t+'_dynamics_VDB'  + '.png', dpi=300)
        else:
            plt.title(t+'\nDynamics of Change in behavior around laser epoch\n Behaviors: Rearing, Other,BTC,Jump')
            plt.savefig(output_folder + '/'+t+'_dynamics_nVDB' + '.png', dpi=300)
        plt.close()
        for behavior in range(num_of_behaviors):
            ...
            plt.ylim([0, 1])
            plt.title(t+'\nDynamics of Change in behavior around laser epoch\n Behaviors: ' + behaviors[behavior])
            plt.savefig(output_folder + '/'+t+'_dynamics_' + behaviors[behavior] + '.png', dpi=300)
            plt.close()
...
for t in ['splashTest1p2mW','splashTest2p2mW']:
    ...
    for mouse in stim_days[t]:
        ...
        for stim_idx in range(stim_times.size):
            ...
            SSD_matrix[m_idx,stim_idx,:]=hf.smoothing(SSD[laser_onset-bin_duration:laser_onset+(2*bin_duration)],5)
    ...
    plt.savefig(output_folder + '/'+t+'_velocity'  + '.png', dpi=300)
    plt.close()
    ...
    plt.savefig(output_folder + '/'+t+'_vigor' + '.png', dpi=300)
    plt.close()

#%% Comparison of vigor on/off laser for all behaviors
x_axis = np.arange(25,150,0.05)
output_folder = root_folder+'/Figures/Optogenetics/A2a_splashTest/Movement_parameters/'
for t in ['splashTest1p2mW','splashTest2p2mW']:
    ...
    for behavior in range(num_of_behaviors):
        ...
        for x in x_axis:
            if len(laser_off[behavior])>0:
                laser_off_cdf.append(np.count_nonzero(laser_off[behavior] <= x) / len(laser_off[behavior]))
            if len(laser_on[behavior])>0:
                laser_on_cdf.append(np.count_nonzero(laser_on[behavior] <= x) / len(laser_on[behavior]))

        if len(laser_off_cdf)>0:plt.plot(x_axis, laser_off_cdf, c='gray', alpha=.7, label='Laser off')
        if len(laser_on_cdf)>0:plt.plot(x_axis, laser_on_cdf, c='blue', alpha=.7, label='Laser on')
        plt.title('CDF')
        plt.ylabel('${P}(|SSD| \leq {x}$)')
        plt.legend()
        plt.suptitle(t+' - SSD statistics for behavior : ' + behaviors[behavior])
        fig.tight_layout(rect=[0.05, 0.05, .9, 0.9], pad=.1)
        plt.savefig(output_folder+t+'_SSD_stat_laser_on_off_'+behaviors[behavior]+'.png', dpi=300)
        plt.close()
    ...
    for category in [VDB, nVDB]:
        ...
        plt.legend()
        if category==VDB:
            plt.title(t+'\nDistribution of vigor index in laser on/off times\n Behavior: Grooming, body licking, wall licking, floor licking')
            plt.savefig(output_folder+t+'_laser_on_off_VDB.png', dpi=300)
        else:
            plt.title(t+'\nDistribution of vigor index in laser on/off times\n Behavior: Rearing, other, BTC, jump')
            plt.savefig(output_folder + t+'_laser_on_off_nVDB.png', dpi=300)
        plt.close()
