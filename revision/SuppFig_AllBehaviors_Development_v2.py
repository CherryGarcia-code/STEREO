#%% SuppFig: All 9 Behaviors Across Cocaine Sessions (v2 — spaghetti plots)
"""
Variant of SuppFig_AllBehaviors_Development where Row 1 (A–I) uses
individual-mouse spaghetti plots + mean±SEM line instead of bar charts.
Panels J–L and M–Q are identical to the original.

Panels:
  Row 1 (A–I): Individual mouse trajectories + mean±SEM for each of 9 behaviors
...
# --- Row 1: Per-behavior spaghetti plots (3x3 grid) ---
...
for b in range(n_behaviors):
    ...
    ax.fill_between(x, mean_vals - sem_vals, mean_vals + sem_vals, color=colors[b], alpha=0.25)
    ax.plot(x, mean_vals, color=colors[b], lw=1.5, marker='o', markersize=2.5)
    ...
    ax.set_xticks(x)
    ax.set_xticklabels(xlabels, fontsize=4.5, rotation=45)
...
save_fig(fig, output_folder, 'SuppFig2v2_AllBehaviors_Development')
